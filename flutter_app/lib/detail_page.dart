...
class _DetailPageState extends State<DetailPage> {
  ...
  Future<void> _deleteTask() async {
    ...
    try {
      final url = Uri.parse(
        'http://10.0.2.2/sche_do_project/backend_api/delete_task.php',
      );
      final response = await http.post(
        url,
        body: {'task_id': widget.task['id'].toString()},
      );
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.pop(
          context,
          true,
        ); // ส่งค่า true กลับไปให้หน้า TaskPage รีเฟรช
      }
    } catch (e) {
      debugPrint("Error: $e");
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    Color pColor = priority.toLowerCase() == 'high'
        ? Colors.red
        : (priority.toLowerCase() == 'medium' ? Colors.orange : Colors.green);
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailRow(
                    ...
                    "Due Date",
                    widget.task['app_date'] ?? '-',
                  ),
                  _buildDetailRow(
                    Icons.access_time_outlined,
                    "Time",
                    widget.task['app_time'] ?? '-',
                  ),
                  _buildDetailRow(
                    Icons.label_outline,
                    "Category",
